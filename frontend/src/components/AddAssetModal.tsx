...
export default function AddAssetModal({ dropdowns, onClose, onCreated }: AddAssetModalProps) {
  ...
  const validate = (): boolean => {
    ...
    if (form.macAddress.trim() && !MAC_ADDRESS_REGEX.test(form.macAddress.trim())) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = (err as { response?: { status?: number; data?: { message?: string; details?: string[] } } })?.response
      let message = response?.data?.message || t('addAssetModal.genericError')
      if (response?.status === 409) {
        message = t('addAssetModal.duplicateError')
      }
      ...
      >
        ...
        {successAsset ? (
          ...
          <>
            ...
            <div style={{ padding: 16, borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: 8, alignItems: 'center' }}>
              {submitError && (
                <div style={{ fontSize: 12, color: '#cf222e', marginInlineEnd: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
                  {submitError}
                </div>
              )}
