    ...
    >
      ...
      >
        <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ fontSize: 15.5, fontWeight: 700, color: 'var(--text-primary)' }}>{t('changePassword.title')}</div>
        ...
        {success ? (
          <div style={{ padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>{t('changePassword.successTitle')}</div>
          ...
          <form onSubmit={handleSubmit} style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              <label style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--text-body)' }}>{t('changePassword.current')}</label>
              <input className="input-base" type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} required autoFocus />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              <label style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--text-body)' }}>{t('changePassword.new')}</label>
              <input className="input-base" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
              ...
              <button type="submit" className="btn-primary" disabled={submitting}>
                {submitting ? t('common.loading') : t('changePassword.submit')}
              </button>
            </div>
          </form>
        )}
